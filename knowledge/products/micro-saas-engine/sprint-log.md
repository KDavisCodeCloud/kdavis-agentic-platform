...
## Sprint 6 — Close the Leads Pipeline Loop + Live Security Fix (2026-09-07)

### Done this sprint
- [x] Applied the gated `mse_leads` CRM ALTER (migration `20260831000035`, staged since 2026-08-31 pending human review) to `microsaas-prod` — `owner`/`stage`/`next_action`/`next_action_due`/`last_activity_at` now live. Table had 0 rows at apply time, zero risk.
- [x] New `update_lead_stage()` / `log_lead_activity()` Postgres functions (migration `20260907000042`) — atomic stage-change + activity-log, mirrors the existing `decide_hitl_item()` shape. `update_lead_stage` is admin-only (matches live `mse_leads` RLS); `log_lead_activity` is admin+hitl.
- [x] **Found and fixed a live security bug while testing the above**, not built by this sprint originally: `decide_hitl_item()` (migration `20260831000036`) checked the caller's role with a bare `<>`/`NOT IN` comparison — `NULL <> 'admin'` is SQL-unknown, not true, so plpgsql's `IF` silently skipped the check for any caller with no role claim at all (i.e. anyone using only the public anon key). An anonymous caller could approve/reject any pending `mse_hitl_items` row, including bypassing the admin-only positioning-approval gate. Fixed live via migration `20260907000043` (`coalesce(v_role, '')`). Full writeup: [[003-null-role-check-bypass]].
- [x] Wired `mse_activities` logging + `stage: new → contacted` advancement into `mkt_lead_finder.py` (on find) and `mkt_o5_sequence_sender.py` (on a real touch_1 send) — non-regressing, only fires from `new`. No automated signal exists anywhere in this codebase for `replied`/`qualified`/`demo`/`won`/`lost` (no inbound-email webhook) — those stay manual, by design, not an oversight.
- [x] New backend endpoints: `GET /marketing/leads/pipeline-summary`, `/icp-products`, `/outreach-summary` (`api/routers/leads.py`).
- [x] `decoded-empire-os`'s `/leads` page is now a real editable pipeline (was hard-gated read-only pending the ALTER above).
- [x] `ceo-dashboard`'s Marketing & Sales page: added a Lead Pipeline panel (manual "Run Lead Finder Now" / "Send Due Sequences Now" — the n8n-outage fallback, proxies to the same backend endpoints those crons call) and replaced both static mocks (Sales Pipeline, Cold Outreach Tracker) with real `mse_leads.stage` / `mse_dm_sequences` counts.
- [x] Full test suite: 551 passed, 1 pre-existing skip. `npx tsc --noEmit` clean on both `decoded-empire-os` and `ceo-dashboard`; both build clean.
- [x] Committed + pushed all three repos (`kdavis-microsaas-engine`, `decoded-empire-os`, `kdavis-agentic-platform`), deployed: `mse-api` (Railway), `empire-dashboard` + `ceo-decoded-dashboard` (Vercel, production).

### Known gap found, not fixed (infra, not app code — fixed live during this sprint's own deploy)
`mse-api`'s Railway GitHub source connection went stale again (same failure mode as the 2026-08-22 sprint) — pushing to `main` didn't auto-deploy. Re-running `connect-service-source` fixed the webhook but also silently reset the service's builder from Nixpacks to Railpack and, critically, **reset `rootDirectory` to `""` (repo root)** — the real root cause. The GitHub repo's actual app code (`requirements.txt`, `api/`, everything) lives one level down at `kdavis-microsaas-engine/`, not at repo root (this local checkout's own git toplevel is the parent directory containing every sibling project, not this repo alone — see [[mse_marketing_engine_architecture]] in Claude's memory). With `rootDirectory=""`, Railpack found no Python project at all and shipped an ~80MB image with no Python runtime installed — which looked like a PATH problem (`uvicorn`/`python`/`python3` all "not found") and cost two wrong fix attempts (a `Procfile`, then `python3 -m uvicorn`) before the actual misconfigured `rootDirectory` was spotted by diffing the build's own "install apt packages: git" log against what a real Python build should show (pip install, venv creation — neither ran). Real fix: `rootDirectory` set to `kdavis-microsaas-engine` via Railway's API. Confirmed live via a real health-check poll against the production URL (`{"status":"ok","commit_sha":"8780ebb2..."}`) and the live OpenAPI schema listing the new endpoints — not just deployment status, which reported `SUCCESS` for the broken builds too.

### Next
- [ ] Watch for `mse-api`'s Railway source connection going stale again — this is the second time; worth checking whether Railway's webhook actually persists correctly for this project or something about this account/repo keeps invalidating it. **If it happens again, check `rootDirectory` is still `kdavis-microsaas-engine` before anything else** — reconnecting the source silently reset it to `""` both times, and that's the actual failure, not the webhook itself.
- [ ] Pick a real lead once the Sunday lead-finder run produces one, walk it through the new editable pipeline by hand (new → contacted happens automatically; contacted → replied → qualified → demo → won/lost is a manual click on decoded-empire-os's `/leads` page)
