...
const PROBLEMS = [
  {
    ...
    a: 'Cloud Decoded triages every alert before it reaches a human — a transient pod restart that self-corrects gets closed automatically with a note, not a page. Only alerts that actually need a decision reach your on-call rotation.',
  },
  {
    q: 'Why does root-cause triage eat so much senior engineering time?',
    a: 'Because reconstructing what happened from raw logs is manual, repetitive work. Cloud Decoded reads the failed pipeline run, surfaces the exact failing step, and proposes the fix in the time it takes a human to open the log tab — turning a 20-minute investigation into a 2-minute approval.',
  },
  {
    q: 'How do you avoid vendor lock-in when adopting infrastructure monitoring and remediation tooling?',
  ...
  {
    q: 'Is it safe to let a monitoring tool execute changes against production infrastructure?',
    a: 'Only if nothing executes without your team approving it first. Cloud Decoded’s approval step is not a configurable setting that can be silently disabled — every proposed fix, for every capability, waits for explicit approval before it touches your infrastructure.',
  },
  {
    q: 'How do you clean up over-permissioned IAM roles without breaking something?',
    a: 'Cloud Decoded diffs what a role actually used against what it’s currently allowed to do, and proposes the tightened policy as a reviewable PR — so the least-privilege cleanup happens on your timeline, with a diff you can read before it ships.',
  },
  {
    q: 'Where does most cloud cost waste actually come from, and how do you find it?',
    a: 'Idle resources nobody remembered to delete — unattached volumes, stopped-but-still-billed instances, unused Elastic IPs. Cloud Decoded reads your real Cost Explorer or Azure Cost Management export and ranks specific, actionable fixes by real monthly savings, not a generic rightsizing report.',
  },
  {
    q: 'How do you keep Terraform state from silently drifting from what’s actually running?',
    a: 'Cloud Decoded compares live infrastructure state against your Terraform, CloudFormation, or Kubernetes manifests continuously, and opens a remediation PR the moment it finds a difference — so drift gets caught in days, not at the next incident.',
  },
  {
    q: 'How do you patch vulnerable dependencies across multiple ecosystems without a manual audit every sprint?',
    a: 'Cloud Decoded scans npm, pip, go, maven, ruby, and cargo manifests against OSV.dev, and proposes a patched manifest with a severity-ranked summary — as a PR, a tracking issue, or both.',
  },
  {
    q: 'How do new on-call engineers get context on an unfamiliar incident fast?',
    a: 'Cloud Decoded answers "what do I do about this" against your team’s actual runbooks and incident history — not a generic wiki search — so a new on-call engineer isn’t starting from zero at 2am.',
  },
  {
    q: 'How do you turn a written runbook into something that actually executes reliably?',
    a: 'Cloud Decoded turns a written procedure into a step-gated workflow: each step runs, reports its result, and waits before the next one fires — so a runbook stops being a document someone has to interpret under pressure.',
  },
]
          ...
          >
            <div style={{ flex: 1, minWidth: 260 }}>
              <h2 style={{ fontFamily: HEAD_FONT, fontWeight: 600, fontSize: 22, color: '#fff', margin: '0 0 8px' }}>
                See every capability in detail.
              </h2>
