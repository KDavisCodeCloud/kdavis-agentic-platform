...
import { approveAuditItem, dismissAuditItem } from '@/lib/api'
import { AUDIT_ITEM_STATUS_META, type RemediationItem, type AuditItemStatus } from '@/lib/types'

interface AuditRemediationItemCardProps {
  item: RemediationItem
  token: string
  onActioned: (itemId: string, status: AuditItemStatus) => void
}
...
export function AuditRemediationItemCard({ item, token, onActioned }: AuditRemediationItemCardProps) {
  const [loading, setLoading] = useState<'approve' | 'dismiss' | null>(null)
  const [error, setError]     = useState<string | null>(null)

  const isPending  = item.status === 'pending_approval'
  const statusMeta = AUDIT_ITEM_STATUS_META[item.status]

  async function handleAction(action: 'approve' | 'dismiss') {
    setLoading(action)
    setError(null)
    try {
      const fn = action === 'approve' ? approveAuditItem : dismissAuditItem
      const result = await fn(token, item.id)
      onActioned(item.id, result.status as AuditItemStatus)
    } catch (e: unknown) {
